...
# Time-stamp: <2009-12-02 23:41:21 Tao Liu>

"""Description: Convert wiggle file to fixed step bedGraph.
...
def main():
    ...
    optparser.add_option("-m","--method",dest="method",type="string",default="median",
                         help="method to process the paired two sets of data in the sampling step. Choices are 'median', 'mean', 'sum' and 'sample' (just take one point out of a data set). Default: median")
    ...
    if method == 'median':
        ...
        medfunc = lambda x: sum(x)/options.step
    elif method == 'sum':
        medfunc = sum
    elif method  == 'sample':
    ...
    for i in range(len(wigfhds)):
        ...
        wig = WiggleIO.WiggleIO(wigfhd).build_binKeeper(chromLenDict=chrom_len,bin=step)
        ...
        for chrom in chroms:
            ...
            for (parray,varray) in bkeeper.cage: # for each cage in binkeeper, there are [position array] and [value array].
                j = j+1
                if varray:
                    datafhd.write("%s\t%d\t%d\t%f\n" % (chrom,(j-1)*step*1000,j*step*1000,medfunc(varray)))
                else:
                    datafhd.write("%s\t%d\t%d\tNA\n" % (chrom,(j-1)*step*1000,j*step*1000))
        wigfhd.close()
        datafhd.close()
